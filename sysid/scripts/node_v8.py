# %% [markdown]
# # NODE v8 — Treinamento Global: Melhor Sistema
#
# Avaliação de 5 arquiteturas de modelo de atrito treinadas em TODOS os dados simultaneamente.
...
# Como os modelos ODE físicos são extremamente leves (3 a 84 params), o cálculo
...
# (ex: testando parâmetros diferentes) sem medo de estourar a memória da placa de vídeo!
...
def train_node(model, name, train_datasets, val_datasets,
    ...
    for epoch in range(1, epochs + 1):
        ...
        for ds in train_datasets:
            # Datasets já estão no device (GPU) — sem transferência CPU→GPU
        ...
        # Detectar quando atingiu k_max pela primeira vez
        if k_steps >= k_max and not reached_kmax:
            reached_kmax = True
            no_improve = 0
            best_val = val_rmse(model, val_datasets)
            best_sd  = {k: v.cpu().clone() for k, v in model.state_dict().items()}
            print(f"    ★ Curriculum completo (k={k_max}). Iniciando early stopping. Val={best_val:.3f}°")

        # Val check + early stopping SÓ após atingir k_max
        if reached_kmax and (epoch % 50 == 0 or epoch == epochs):
        ...
        if reached_kmax and no_improve >= patience:
...
if __name__ == '__main__':
    ...
    out_dir   = f"resultados_v8_{timestamp}"
    ...
    print("Carregando Val e Teste na GPU...")
    ...
    for tipo, ds_list in test_por_tipo.items():
        all_test_ds_cpu.extend([{'name': d['name'], 't': d['t'].cpu(), 'u': d['u'].cpu(), 'x': d['x'].cpu()} for d in ds_list])
    plot_datasets(all_test_ds_cpu, 'Teste (cross-session)',
                  out_path=f"{out_dir}/dados_teste.png")
    del train_ds_cpu, val_ds_cpu, all_test_ds_cpu
    ...
    for i, mod_nome in enumerate(mod_nomes, 1):
        ...
        compiled_model, best_val = train_node(
            ...
            epochs=4500, lr=0.015,
            k_min=20, k_max=1200, curriculum_stage_epochs=400, patience=1000
        )
        ...
        for test_tipo, test_ds in test_por_tipo.items():
            res = avalia_free_run(compiled_model, test_ds)
            ...
            print(f"  [{tag} TESTE {test_tipo:<10}] RMSE={rmse_m:.3f}° R²={r2_m:.4f} FIT={fit_m:.1f}%")
        ...
        print(f"  [{tag} MÉDIA GERAL] RMSE={rmse_g:.3f}° R²={r2_g:.4f} FIT={fit_g:.1f}%")
    ...
    print("[V8] Treinamento Global concluído.")
